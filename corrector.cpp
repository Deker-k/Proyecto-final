...
void insertarDic(char* palabra, char dic[][TAMTOKEN], int est[], int& n) {
    if (strlen(palabra) == 1 && simboloProhibido(palabra[0])) return;
    int l = 0, r = n - 1, mid, c;
    while (l <= r) {
        mid = (l + r) / 2;
...
void agregar(char* c, char s[][TAMTOKEN], int& n) {
    if (n < MAX_SUGERENCIAS) { strcpy(s[n], c); n++; }
}
